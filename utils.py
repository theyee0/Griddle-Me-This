...
def chess_to_tensor(board):
    ...
    pieces = tuple(map(board.piece_at, chess.SQUARES))
    ...
    piece_tensor = lambda x, y: torch.Tensor(list(map(piece_selector(chess.Piece(x, y)), pieces))).reshape(8, 8)

    # Returns a list of all 8x8 tensors for both black and white pieces
    subtensors = [piece_tensor(x, y) for x in types for y in colors]

    # Returns the list of tensors as a tensor itself
    return torch.stack(subtensors)
...
def position_to_tensor(position):
    ...
    pieces = list(map(lambda x: position == x, chess.SQUARES))
